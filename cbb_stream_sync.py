"""
cbb_stream_sync.py — College Basketball Stream Sync
=====================================================
Mirrors nba_stream_sync.py for CBB data via ESPN API.

Setup:
  1. In your Google Sheet, add a tab named "CBB"
  2. Copy the same column structure from your NBA sheet (A-L)
  3. Fill in player rows; set Is_Live=TRUE for the player you want to feature
  4. Run: python cbb_stream_sync.py
"""
...
REFRESH_SECONDS = 5     # Match overlay poll rate
DEFAULT_NUM_GAMES = 30

# Column indices (match your NBA sheet layout exactly)
COL_NAME       = 1
COL_SCORE      = 2
COL_AVG        = 3
COL_TYPE       = 4
...
def clean_sheet(worksheet, live_row, total_rows, player_name, stat_type):
    """Set live/featured flags for the active row; clear all others."""
...
def main():
    print("--- CBB Stream Sync Started ---")
    worksheet = connect_to_sheet()

    all_rows = worksheet.get_all_values()
    live_row, player_name, stat_type, avg_target = -1, None, None, 0.0

    for i, row in enumerate(all_rows[1:], start=2):
        if len(row) > 4 and row[COL_IS_LIVE - 1].upper() == 'TRUE':
            live_row    = i
            player_name = row[COL_NAME - 1].strip()
            stat_type   = row[COL_TYPE - 1].strip().upper() or 'PTS'
    ...
    print(f"Streaming: {player_name} — {stat_type}")

    stats = get_cbb_stats_raw(player_name, stat_type, num_games=DEFAULT_NUM_GAMES)
    if not stats:
        print(f"Could not fetch stats for '{player_name}'. Check name spelling.")
        return
